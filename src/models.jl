...
@doc PyDoc(WJFKeras._models, :Sequential) struct Sequential <: WJFKeras.Model
    obj::PyObject
    layers::Array{WJFKeras.Layer}

    function Sequential()
        new(WJFKeras._models.Sequential(), WJFKeras.Layer[])
    end

    function Sequential(layers::Array{WJFKeras.Layer})
        new(
            WJFKeras._models.Sequential([PyObject(l) for l in layers]),
            layers
        )
    end
end

WJFKeras.layers(m::Sequential) = m.layers

WJFKeras.inputs(m::Sequential) = [Tensor(i) for i in m.obj.inputs]

WJFKeras.outputs(m::Sequential) = [Tensor(o) for o in m.obj.outputs]

Base.getindex(m::Sequential, i::Int) = m.layers[i]

function add!(m::Sequential, l::WJFKeras.Layer)
    push!(m.layers, l)
